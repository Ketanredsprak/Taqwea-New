@if (!empty($data) && isset($data))
    <div class="chatBox common-shadow mx-auto">
            ...
            <div class="d-flex">
                <div class="userInfo">
                    <div class="userInfo_img">
                        <img src="{{ $data->student->profile_image_url }}" alt="{{ $data->student->name }}">
                    </div>
                </div>
                <div class="userView">
                    <div class="d-flex align-items-center justify-content-between flex-wrap">
                    <div class="userView__cnt">
                        <h5 class="font-bd">{{ $data->student->name }}</h5>
                        <p class="mb-0 font-sm">{{ $data->class->class_name }}</p>
                    </div>
        ...
        <div class="chatBox-body" id="chatDetailList" nice-scroll data-chat-user="{{$data->student->id}}" data-thread-uuid="{{$data->uuid}}">
        ...
        <form action="javascript:void(0);">
                <div class="d-flex ">
                    <input type="hidden" class="form-control" id="sendMessage_fromId" name="fromId" value="{{$data->tutor->id}}">
                    <input type="hidden" class="form-control" id="sendMessage_toId" name="toId" value="{{$data->student->id}}">
                    ...
                    <input type="text" class="form-control"   id="sendMessage_message" name="message" placeholder="{{__('labels.type_here')}}">
