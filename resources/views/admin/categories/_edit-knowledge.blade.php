<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">{{$category ? "Update" : 'Add'}} Category</h5>
            <a href="#" class="close" data-dismiss="modal" aria-label="Close">
                <em class="icon ni ni-cross"></em>
            </a>
        </div>
        <div class="modal-body">
            <form method="{{$category ? "PUT" : 'POST'}}" id="category-frm" action="{{ $category ? route('categories.update', ['category' => $category->id]) : route('categories.store') }}" class="form-validate is-alter">
                {{csrf_field()}}
                <input type="hidden" name="parent_id" value="{{$parent->id}}" />
                <input type="hidden" id="category-id" name="id" value="{{$category ? $category->id : null}}" />
                <div class="col-12">
                    <div class="form-group">
                        <div class="upload_photo mb-2 mb-md-3 mx-auto text-center">
                            <div class="img-box">
                                <img src="{{ $category ? $category->icon_url : null }}" alt="General-Knowledge"
                ...
                <div class="mt-4 text-center">
                    <button type="submit" id="category-btn" class="btn btn-lg btn-primary">{{$category ? "Update" : 'Add'}} Category</button>
                </div>
            </form>
            {!! JsValidator::formRequest('App\Http\Requests\Admin\UpdateCategoryRequest','#category-frm') !!}
        </div>
    </div>
</div>