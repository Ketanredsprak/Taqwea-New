                        ...
                        <div class="row">
                                    ...
                                    <div id="faqs" class="accordion customAccordion">
                                        ...
                                        <div class="accordion-item commonBox">
                                            <div class="accordion-head w-100 text-left bg-white" data-target="#faq-q{{ $key }}" data-toggle="collapse">
                                                ...
                                                @endif
                                                <div class="action">
                                                    <buttton class="accordion-icon" data-toggle="collapse" data-target="#faq-q{{ $key }}" title="{{ __('labels.view_answer') }}">
                                                        {{ $loop->iteration }}: {{ $topic->translateOrDefault()->topic_title }}
                                                    </buttton>
                                                </div>
                                            </div>
                            ...
                            <div class="col-lg-4">
                                <div class="subject__right subject--box commonBox">
                                    <div class="classImg">
                                        <img src="{{ $class->class_image_url }}" alt="{{ @$class->tutor->translateOrDefault()->name }}" class="img-fluid class-img" />
                                    ...
                                    <div class="amountInfo d-flex justify-content-between">
                                        ...
                                        <div class="amountInfo__right">{{ __('labels.sar') }}  <span style="text-transform: text-uppercase;">{{ (!empty($class->total_fees))?number_format($class->total_fees, 2): number_format(round(($class->duration/60)*$class->hourly_fees,2),2) }}</span></div>
                                    </div>
                                    <div class="dateTime d-flex align-items-center">
                                        <em class="icon-calender"></em>
                                        <p class="mb-0" dir="{{config('constants.date_format_direction')}}">{{ convertDateToTz($class->start_time, 'UTC', 'd M Y') }}<span> {{ convertDateToTz($class->start_time, 'UTC', 'h:i A') }}</span></p>
                                    </div>
                                    <div class="btnRow d-flex justify-content-between">
                                        ...
                                        @if(!$genderBooking)
                                        <div class="col-12">
                                            <div class="alert alert-danger">{{ __('error.class_booking_gender_wise', ["gender"=>$class->gender_preference, "class" => $class->class_type]) }}</div>
                                        </div>
                                        @endif


                                        @if($class->status == 'completed')
                                        <a class="btn btn-primary btn-primary--outline ripple-effect btn-lg" href="{{route('student.feedback.index', ['class' =>$class->id])}}" tabindex="0">{{__('labels.my_feedback')}}</a>
                                        @if($isChat)
                                        <a class="btn btn-primary ripple-effect btn-lg" href="{{route('message-student')}}?class={{$class->id}}" tabindex="0">{{__('labels.chat')}}</a>
                                        @else
                                        ...
                                        <a class="btn btn-primary ripple-effect btn-lg w-100 mr-2 join-now disabled" data-start-time="{{ $class->start_time }}" href="{{ route('student.join', ['slug' => $class->slug]) }}" tabindex="0">{{__('labels.join_now')}}</a>
                                        @endif
                                        @if(in_array(@$class['bookings'][0]['status'], ['confirm', 'pending']))
                                        <a href="Javascript:void(0)" onclick="cancelBooking($(this),{{ @$class['bookings'][0]['id'] }})" class="btn btn-primary btn-primary--outline ripple-effect btn-lg cancel-btn-disabled">{{__('labels.cancel')}}</a>
                                        @endif

                                        @elseif($genderBooking)
                                        @if((Auth::check() && !empty(@$class->cartItem)) || !($class->is_booking))
                                        <button class="btn btn-primary btn-primary--outline ripple-effect btn-lg add-to-cart" data-start-time="{{ classBookingBefore($class->start_time) }}" tabindex="0" disabled>{{ __('labels.add_to_cart') }}</button>
                                        @elseif(Auth::check())

                                        <button class="btn btn-primary btn-primary--outline ripple-effect btn-lg add-to-cart disabled" data-start-time="{{ classBookingBefore($class->start_time) }}" tabindex="0" onclick="addToCart($(this), {{ $class->id }}, 'class')">{{ __('labels.add_to_cart') }}</button>
                                        @else
                    ...
                    <div class="col-lg-3">
                        <div class="tutorProfile__left">
                            <div class="tutorProfile__teacherImg">
                                <img src="{{ @$class->tutor->profile_image_url }}" alt="{{ __('labels.tutor_profile') }}">
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-9">
                        <div class="tutorProfile__right">
                            ...
                            <div class="d-flex align-items-center justify-content-lg-start justify-content-center">
                                <div class="rateStar w-auto" data-rating="{{(@$class->rating->rating?@$class->rating->rating:'0')}}"></div>
                            </div>
                            <p>{{ @$class->tutor->translateOrDefault()->bio }}</p>
                            <a class="btn btn-primary ripple-effect" href="{{ route('featured.tutors.show', ['tutor' => $class->tutor_id]) }}" tabindex="0">{{ __('labels.view_profile') }}</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                @forelse ($shareLinks as $provider => $link)
                    ...
                    @if ($loop->last)
                        <li><a href="mailto:?body={{ __('message.share_message_text', ['type' => $typeLabel])}}%0D%0A{{$url}}"><em class="icon-email"></em></a></li>
                        </ul>
                    @endif
                @empty
