$("#avatar").change(function(e) {
    ...
    if (frm.valid()) {
        ...
        $.ajax({
            type: "POST",
            data: formData,
            url: process.env.MIX_APP_URL + '/accountant/upload-profile',
            datType: 'JSON',
            contentType: false,
            processData: false,
            success: function(response) {
                if (response.success) {
                    ...
                    setTimeout(function() {
                            window.location.href = process.env.MIX_APP_URL + '/accountant/profile';
                        },
                        2000);
                }
            },
...
$("#submit-btn").on('click', (function(e) {
    ...
    if (frm.valid()) {
        ...
        $.ajax({
            url: process.env.MIX_APP_URL + '/accountant/update-profile',
            type: "POST",
            data: frm.serialize(),
            dataType: 'JSON',
            success: function(response) {
...
$("#change-password-btn").on('click', (function(e) {
    ...
    if (frm.valid()) {
        showButtonLoader(btn, btnName, 'disabled');
        $.ajax({
            url: process.env.MIX_APP_URL + '/accountant/changePassword',
            type: "POST",
            data: frm.serialize(),
            success: function(response) {
                if (response.success) {
